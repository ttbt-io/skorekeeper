...
const mockDB = {
    ...
    loadGame: jest.fn().mockResolvedValue({ id: '123', actionLog: [] }),
...
describe('Routing Logic', () => {
    ...
    beforeEach(() => {
        ...
        // Mock loadGameForView to avoid complex rendering logic
        jest.spyOn(app, 'loadGameForView').mockImplementation(async(id, view, scoresheetView = 'grid') => {
            // Simulate what loadGameForView does minimally
            app.state.activeGame = { id: id };
            app.state.view = view;
            if (view === 'scoresheet') {
                app.state.scoresheetView = scoresheetView;
            }
        });

        // Mock sync
        app.sync = { disconnect: jest.fn() };
        app.renderSyncStatusUI = jest.fn();
        app.loadDashboard = jest.fn();
    });
    ...
    test('handleHashChange #game/ route should default to grid view', async() => {
        window.location.hash = '#game/123';
        app.state.scoresheetView = 'feed'; // Set to something else first

        await app.handleHashChange();

        expect(app.state.scoresheetView).toBe('grid');
        expect(app.loadGameForView).toHaveBeenCalledWith('123', 'scoresheet', 'grid');
    });

    test('handleHashChange #feed/ route should set feed view', async() => {
        window.location.hash = '#feed/456';
        app.state.scoresheetView = 'grid'; // Set to default

        await app.handleHashChange();

        expect(app.state.scoresheetView).toBe('feed');
        expect(app.loadGameForView).toHaveBeenCalledWith('456', 'scoresheet', 'feed');
    });

    test('handleHashChange #broadcast/ route', async() => {
        window.location.hash = '#broadcast/789';

        await app.handleHashChange();

        expect(app.loadGameForView).toHaveBeenCalledWith('789', 'broadcast');
    });
});
